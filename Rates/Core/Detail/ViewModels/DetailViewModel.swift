...
class DetailViewModel: ObservableObject {
    ...
    private func addSubscribers() {
        ...
        coinDetailService.$coinDetails
            ...
            .sink { [weak self] (retuenedArrays) in
                self?.overviewSratistic = retuenedArrays.overview
                self?.additionalSratistic = retuenedArrays.additional
            }
            .store(in: &cancellables)
        
        coinDetailService.$coinDetails
            .sink { [weak self] (retuenedCoinDetails) in
                self?.coinDescription = retuenedCoinDetails?.readableDescription
                self?.webSiteURL = retuenedCoinDetails?.links?.homepage?.first
                self?.redditURL = retuenedCoinDetails?.links?.subredditURL
            }
            .store(in: &cancellables)
    ...
    private func mapDataToStatistics(coinDetailModel: CoinDetailModel?, coinModel: CoinModel) -> (overview: [StatisticModel], additional: [StatisticModel]) {
        ...
        let overviewArray = createOverviewArrey(coinModel: coinModel)
        let additionalArray = createAdditionalArray(coinModel: coinModel, coinDetailModel: coinDetailModel)
        
        return (overviewArray, additionalArray)
    }
    
    func createOverviewArrey(coinModel: CoinModel) -> [StatisticModel] {
